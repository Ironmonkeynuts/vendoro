...
def create_checkout_session(request):
    ...
    if not settings.STRIPE_SECRET_KEY or not settings.STRIPE_PUBLIC_KEY:
        messages.error(request, "Payment is not configured. Please try again later.")
    ...
    if len(shops) != 1:
        messages.error(
            request, "Multi-shop checkout not yet supported. Please keep items from one shop."
        )
        return redirect("orders:cart_detail")
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            success_url=_abs_url(request, "payments:success") + "?session_id={CHECKOUT_SESSION_ID}",
        ...
        messages.error(request, "Payment configuration error. Please try again later.")
        return redirect("orders:cart_detail")
        ...
        order.status = Order.Status.CANCELED
        order.save(update_fields=["status"])
        messages.error(request, getattr(e, "user_message", "Unable to start checkout."))
        return redirect("orders:cart_detail")
...
def success(request):
    ...
    if session_id:
        ...
        if session:
            ...
            order = (
                Order.objects.filter(stripe_checkout_session_id=session.id).first()
                or Order.objects.filter(id=session.get("metadata", {}).get("order_id")).first()
            )

            # Idempotently mark as paid if Stripe says it’s paid
            if order and ((session.get("status") == "complete") or (session.get("payment_status") == "paid")):
                changed = False
                ...
                if session.get("amount_total") is not None:
                    ...
                    if reported != order.total_amount:
                        logger.warning(
                            ...
                            extra={"order_id": order.id, "expected": str(order.total_amount), "reported": str(reported)},
                        )
                    order.total_amount = reported
                    changed = True
                if hasattr(order, "stripe_payment_intent"):
                    pi = session.get("payment_intent") or ""
                    if pi and getattr(order, "stripe_payment_intent", "") != pi:
            ...
            if cart_id:
                try:
                    cart = Cart.objects.get(id=cart_id, user=request.user, active=True)

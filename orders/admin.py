...
class OrderAdmin(admin.ModelAdmin):
    ...
    def mark_fulfillment_pending(self, request, queryset):
        ...
        self.message_user(request, f"Updated fulfillment to Pending on {updated} orders.")
    mark_fulfillment_pending.short_description = "Fulfillment → Pending"

    def mark_fulfillment_processing(self, request, queryset):
        updated = queryset.update(fulfillment_status=FulfillmentStatus.PROCESSING)
        self.message_user(request, f"Updated fulfillment to Processing on {updated} orders.")
    mark_fulfillment_processing.short_description = "Fulfillment → Processing"

    def mark_fulfillment_completed(self, request, queryset):
        updated = queryset.update(fulfillment_status=FulfillmentStatus.COMPLETED)
        self.message_user(request, f"Updated fulfillment to Completed on {updated} orders.")
    ...
    def mark_status_pending(self, request, queryset):
        ...
        self.message_user(request, f"Updated status to Pending on {updated} orders.")
    ...
    def mark_status_paid(self, request, queryset):
        ...
        self.message_user(request, f"Updated status to Paid on {updated} orders.")
    ...
    def mark_status_canceled(self, request, queryset):
        ...
        self.message_user(request, f"Updated status to Canceled on {updated} orders.")
